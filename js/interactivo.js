...
let datos = [];
function guardar(){
    total = miStorage.getItem("total")
    let totaltemp = JSON.parse(total)
    if(totaltemp == null){
        let hora = document.getElementById("inpHoras").value
        let valor = document.getElementById("inpValor").value
        let multiplicacion = (valor*hora);
        datos.push(multiplicacion)
        miStorage.setItem("total", JSON.stringify(datos))
    }else{

        let hora = document.getElementById("inpHoras").value
        let valor = document.getElementById("inpValor").value
        let multiplicacion = (valor*hora)
        totaltemp.push(multiplicacion)
        miStorage.setItem("total", JSON.stringify(totaltemp))
    }

}

function mostrar(){
    let total = miStorage.getItem("total")
    let totaltemp = JSON.parse(total)
    totaltemp.forEach((e, index) => {
        const div = document.createElement("p");
        div.textContent = "El valor a pagar al empleado No. "+ (index+1) +"  es de: " + e;
        document.getElementById("Resultados").appendChild(div)
    });
}

function eliminar(){
    miStorage.removeItem("total")
}
function graficas1 (){
    Highcharts.chart('container-1', {
        title: {
            text: 'Alimentos de los Residentes'
        },
        xAxis: {
            categories: ['Manzanas', 'Naranjas', 'Peras', 'Bananas', 'Granadilla']
        },
        labels: {
            items: [{
...
function suma (){
    let n1 = document.getElementById("inpHoras").value;
    let n2 = document.getElementById("inpValor").value;
    let suma = parseInt(n1) + parseFloat(n2);
    alert(suma);
}
function date(){
  let municipio = "Abriaqu\u00ed"
    alert(municipio)
}
